...
For v0.6/v0.7 NACC Dry-Run Ingestion work, use `scripts/make_header_samples.py` to create header-only or explicitly requested small sample copies, then use `scripts/scan_nacc_files.py --real-data-mode` on real NACC folders or samples. Produce file inventory, concept coverage, mapping candidates, readiness report, unresolved human questions, and a human confirmation worksheet. Do not output row-level patient records, and do not mark real NACC data as execution-ready from dry-run evidence alone.
