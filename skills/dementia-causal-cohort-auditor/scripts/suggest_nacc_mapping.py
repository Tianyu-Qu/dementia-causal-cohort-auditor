...
CONCEPTS = [
    ...
    {
        "concept_id": "medication_records",
        "label": "Medication or ADRD treatment records, not causal-ready exposure",
        "required": False,
        "grain": "visit",
        "candidates": ["DRUGID", "DRUGNAME", "MEDICATION", "NACCA4", "A4", "A4A"],
        "timing": "records only; must be transformed into an exposure definition before treatment-effect estimation",
    },
    {
        "concept_id": "medication_temporality_support",
        "label": "Medication timing support for exposure construction",
        "required": False,
        "grain": "visit",
...
def emit_mapping(dictionary: dict[str, str], source: Path) -> str:
    ...
    lines.extend(
        [
            ...
            "  - Treat medication variables as records first; confirm whether they support new-user, washout, grace-period, and lag-period definitions before treatment-effect estimation.",
            "",
